...
TourismClient = function(data) {
	var mapping = mapConvert(data), version, rawResources = data;
	this.lastCall = "";
	...
	makeQueryCall = function(resource, parameters, handleData, handleError) {
		...
		lastCall = encodeURI(UriTemplate.build(parameters));
		$.support.cors = true;
		$.ajax({
			headers: { 
